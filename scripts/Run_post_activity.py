...
FABRIC_TOKEN = args.FABRIC_TOKEN or ""
...
def main():
    ...
    if FABRIC_TOKEN!="":
        ...
        token = FABRIC_TOKEN
        # Log token diagnostics for debugging (without exposing the full token)
        token_preview = token[:20] + "..." if len(token) > 20 else token
        logging.info(f'Token preview (first 20 chars): {token_preview}')
